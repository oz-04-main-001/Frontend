...
- <b>UI</b>
  - 페이지 : 메인, 숙소 디테일, 객실 디테일, 예약하기, 사업자 정보 등록
  - 공통 컴포넌트 : 메인 숙소 카드, 버튼, 숙소 예약 정보 카드
- <b>기능</b>
  1. 메인
      -  랜덤한 숙소 리스트 렌더링 합니다.
  2. 검색
      -  도/시, 체크인, 체크아웃, 숙박 인원을 받아 예약 가능한 객실이 있는 숙소를 렌더링 후 지도에 표시 해줍니다.
      -  카카오 지도에 있는 숙박 업소 리스트를 렌더링 합니다.
      -  지도를 보여줍니다.
  3. 디테일
     <br/> 숙소 디테일에서 숙소 정보를보여줍니다. <br/>
      객실 디테일에서 객실 정보를보여줍니다.<br/>
      - 로그인 한 유저
  ...
  4. 마이페이지
     - 호스트 등록/숙소 관리 페이지로 이동합니다.
     - 유저 정보를 렌더링합니다
     - 유저의 숙박 예약 및 이용 정보 리스트를 렌더링 합니다
       - 이용 전까지 예약 취소가 가능합니다.
  5. 호스트 등록
     - guest 타입의 유저가 호스트 등록 시, 사업자 정보 입력 페이지로 이동합니다.
...
- <b>UI</b>
  - 페이지 : 로그인, 로그아웃, 회원가입, 이메일인증, 숙소유형선택(독채, 일반), 숙소건물유형선택
...
- <b>기능</b>
  ...
  - 숙소유형선택(독채, 일반): 하나의 숙소를 한팀이 전부 사용해요(독채), 하나의 숙소에 객실이 여러개 있어요(일반), 둘중에 하나를 선택하면, 선택한 상태를 전역으로 관리해서 건물유형선택에 전달
  - 숙소건물유형선택: '펜션','아파트','빌라','단독주택','호텔','아파트2','빌라2' 일반인지, 독채인지에 대한 상태에 따라, 예) 독채펜션 or 펜션 으로 전역 상태로 저장하게 됩니다.
...
### 한기선 (부팀장)
프로젝트를 진행하면서 처음으로 팀장을 하게 되어 부담스러웠습니다. 일정 관리와 테스크 분배가 어려웠고, 결과적으로 초반에 정했던 MVP를 기한 내 완성하지 못해서 아쉬웠습니다.
백엔드와의 소통을 하면서 서버에 대한 이해가 협업에서 중요하다고 느꼈습니다. 함께 하는 작업이니 소통을 하면서 코드 짜는 시간 확보의 어려움을 깨달았습니다. 
그래도 끝나는 날이 오고 밤새며 배포 작업과 문서작업을 하면서, 프로젝트에 대한 큰 흐름을 겪을 수 있어서 다음에는 더 효율적이고, 개선된 플로우를 가져갈 수 있을것같습니다.
